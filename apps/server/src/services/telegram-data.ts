...
import { TelegramDonatePlan } from "./telegram-types.js";
...
export async function getAllConfiguredVipChatIds() {
  const channels = await prisma.telegramVipChannel.findMany({
    where: { isActive: true },
    select: { chatId: true },
  });
  return Array.from(
    new Set([env.TELEGRAM_VIP_CHAT_ID, ...channels.map((channel) => channel.chatId)].filter(Boolean)),
  );
}

export async function getVipChatIdsForPlan(planCode?: string) {
