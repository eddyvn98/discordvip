...
export class PaymentService {
  ...
  private async processNormalizedPayment(normalized: NormalizedSepayPayload) {
    ...
    if (!applied.duplicate) {
      ...
      try {
        await adapter.grantAccess({ ...target, planCode: order.plan.code });
      } catch (error) {
  ...
  async resolvePendingPayment(paymentId: string, orderCode: string) {
    ...
    await this.platformRegistry.get(target.platform).grantAccess({
      ...target,
  ...
  async getDashboardSummary(platformQuery?: string) {
    const platform = platformQuery === "telegram" ? "telegram" : platformQuery === "all" ? "all" : "discord";
    const orderWhere =
      platform === "discord"
        ? ({
